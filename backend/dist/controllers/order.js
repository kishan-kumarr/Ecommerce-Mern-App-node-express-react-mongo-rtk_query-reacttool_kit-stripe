...
export const createOrder = async (req, res, next) => {
    try {
        ...
        orderItems.map(async (item) => {
            ...
            if (!product)
                return next(new ErrorHandler(`Product not found with this id: ${item?.productId}`, StatusCodes.NOT_FOUND));
...
export const myOrders = async (req, res, next) => {
    try {
        ...
        if (!userId)
            return next(new ErrorHandler(`Please provide user id`, StatusCodes.BAD_REQUEST));
