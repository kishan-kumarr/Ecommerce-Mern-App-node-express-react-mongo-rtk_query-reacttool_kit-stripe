...
export const getSingleProduct = async (req, res, next) => {
    try {
        ...
        if (!product)
            return res.status(StatusCodes.NOT_FOUND).json({ sucess: false, message: `Product not found with this ${_id}` });
...
export const deleteProduct = async (req, res, next) => {
    try {
        ...
        const product = await Product.findById({ _id });
        if (!product)
            return res.status(StatusCodes.NOT_FOUND).json({ sucess: false, message: `Product not found with this id: ${_id}` });
